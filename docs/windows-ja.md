...
## Coordinator/Manager Service パフォーマンスモニターレポートの作成方法

Coordinator/Manager Service の高い CPU 使用率やメモリ消費が原因となって、Coordinator が無応答・再起動する場合があります。これらの挙動を分析するために、一定期間の CPU およびメモリ使用状況を確認することが非常に有効です。ここではすべての Windows システムに標準搭載されている **パフォーマンスモニター（perfmon）** を使ってこれらのデータを取得する方法を説明します。

設定手順

1. パフォーマンスモニターの起動: Windows スタートメニューで「perfmon」と入力し実行します。
2. 新規作成: 「データ コレクター セット」>「ユーザー定義」を右クリックし、「新規作成」>「データ コレクター セット」を選択します。
3. 名前とテンプレート: 任意の名前（例：Incredibuild Coordinator）を入力し、「テンプレートから作成する」を選択して「次へ」をクリックします。
4. テンプレートの選択: 「参照」から[ここにある XML テンプレートファイル](../settings/PerfmonCoordinatorTemplate.xml)をダウンロード・選択して「次へ」をクリックします。
5. 保存場所: （必要に応じて）データの保存先を指定し、「次へ」をクリックします。
6. 保存して閉じるを指定し、「完了」をクリックします。

データの収集と送付

1. 開始: 作成したセットを右クリックして「開始」を選択します。
2. 停止: 十分な時間が経過し、再現が確認できたら（作成したセットを右クリックして）「停止」を選択します。
3. 送付: 「レポート」>「ユーザー定義」から生成されたレポートを右クリックし、「表示」>「フォルダー」からファイルを確認して、該当のファイル（*.blg）を送付してください。

（補足）データの確認方法
- 青: Coordinator のメモリ使用量 (scale = 0.00000001)
- 黄: Manager のメモリ使用量 (scale = 0.00000001)
- 赤: Coordinator の CPU 使用量 (scale = 0.1)
- 緑: Manager の CPU 使用量 (scale = 1.0)
